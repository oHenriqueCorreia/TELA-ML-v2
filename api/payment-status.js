...
export default async function handler(request, response) {
  ...
  try { const result = await triboPayFetch("/transactions/" + encodeURIComponent(identifier)); const transaction = result.data && (result.data.data || result.data.transaction || result.data); if (!result.response.ok || !transaction) return sendJson(response, { error: result.data.message || "Não foi possível consultar a cobrança." }, result.response.status === 404 ? 404 : 502); return sendJson(response, { identifier, status: transaction.status || transaction.payment_status, amount: transaction.amount }); }
